...
function hfc_display(props,atoms,scaling,conmatrix,options)
...
for n=idx
    ...
    hfc=props.hfc.full.matrix{n};

    % Eliminate the isotroic part
    if options.kill_iso
        hfc=hfc-eye(3)*trace(hfc)/3;
    end
    ...
    switch options.style
        ...
        case 'ellipsoids'
            ...
            [eigvecs,eigvals]=eig(hfc,'vector');
        ...
        case 'harmonics'
            ...
            [rank0,rank1,rank2]=mat2sphten(hfc);
...
if (~isfield(props,'hfc'))||(~isfield(props.hfc,'full'))||...
   (~isfield(props.hfc.full,'matrix'))
    error('props structure does not contain hyperfine coupling matrices.');
end
if iscell(atoms)
...
% from Southampton's hapless HR department.
%
