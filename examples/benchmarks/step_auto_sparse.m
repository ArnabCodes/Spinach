% Benchmark for sparse matrix propagation backends. Compares the
...
%
% <https://spindynamics.org/wiki/index.php?title=step_auto_sparse.m>

function step_auto_sparse()
...
for n=1:numel(dims)
    ...
    for k=1:numel(norms)
        ...
        % Generate a random sparse Hamiltonian
        H=sprandn(dims(n),dims(n),10/dims(n))+1i*sprandn(dims(n),dims(n),10/dims(n));
